...
fetch("http://127.0.0.1:5000/init_access")
  ...
  .then((data) => {
    ...
    if (data.initial_access && data.initial_access.ip) {
      ...
      document.getElementById("mac").textContent = `MAC: ${data.initial_access.mac_address}`;
      document.getElementById("machine").textContent = `Nom Machine: ${data.initial_access.nom_machine}`;
      document.getElementById("user").textContent = `Utilisateur: ${data.initial_access.nom_utilisateur}`;
  ...
  .then((data) => {
    ...
    const tentativesDacces = Array.isArray(data.tentatives_dacces_entre_ips_privees) 
  ...
  .then((data) => {
    ...
    const publicTableBody = document.querySelector("#public-tentatives-table tbody");
  ...
  expandableCells.forEach(cell => {
    ...
    if (cell.scrollWidth > cell.clientWidth) {
      ...
      expandButton.onclick = () => {
        ...
        expandButton.textContent = cell.classList.contains("expanded") ? "Afficher moins" : "Afficher plus";
      };
